...
// let the ghost not to go backward
Directions no_backward(Directions i) {
	if (i == UP) return DOWN;
	if (i == DOWN) return UP;
...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M) {
	...
	// if (!ghost_movable(ghost, M, ghost->objData.preMove, ghost->status)) {
		int cnt = 0;
		for (Directions i = 1; i <= 4; i++)
			if (ghost_movable(ghost, M, i, ghost->status) && i != no_backward(ghost->objData.preMove)) 	proba[cnt++] = i;
		if (cnt != 0) ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt - 1)]); //problem
		else ghost_NextMove(ghost, no_backward(ghost->objData.preMove));
	// }
	
	

	// [TODO] (Not in Hackathon) finished!
	// Description:
