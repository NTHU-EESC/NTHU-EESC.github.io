...
/* ----------------------------- start of default map code*/
	...
	if (filepath == NULL) {
		...
		game_log("%s\n", filepath);
		if ((pFile = fopen(filepath, "r")) == NULL) { // serious problem
			game_abort("error to open map file: %s\n", filepath);
			return NULL;
		}
		if (!pFile) {
	...
	*/
	...
	for (int i = 0; i < M->row_num; i++) {
		M->map[i] = (char*)malloc(sizeof(char) * (M->col_num + 5));
		if(!M->map[i]){
			game_abort(/*stderr, */"map char array malloc error\n");
			return NULL;
		}
		...
		for (int j = 0; j < M->col_num; j++) {
			...
			//	M->map[i][j] = nthu_map[i][j];
				...
				if (fscanf(pFile, "%c", &M->map[i][j]) != 1) {
					game_abort("error to open other maps\n");
					return NULL;
				}
				switch (M->map[i][j]) {
				case '#':
