...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money, int _level) :
	Engine::Sprite(img, x, y), speed(speed), hp(hp), money(money), maxSpeed(speed), slowRemainTime(0), level(_level) {
...
void Enemy::Hit(float damage) {
	...
	if (hp <= 0) {
		...
		/*for (auto& it : lockedBullets)
			it->Target = nullptr;*/
		if (!pass) {
			getPlayScene()->EarnMoney(money - level + 1);
			if (level > 1) Upgrade();
		}
...
void Enemy::Upgrade() {
	level--;
	Enemy* enemy = nullptr;
	getPlayScene()->EnemyGroup->AddNewObject(enemy = new DiceEnemy(Position.x, Position.y, level));
