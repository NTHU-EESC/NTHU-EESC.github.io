...
void printForest(Problem2 P, Forest F) {
	std::cout << "=========" << '\n';
	for (auto f : F.trees) {
		std::cout << "ID = " << f.id << '\n';
...
int main(void) {
	...
	FILE* in = freopen("Problem2_test_case.txt", "r", stdin);
	// FILE* in = freopen("sample_input.txt", "r", stdin);
	FILE* out = freopen("Problem2_output.txt", "w", stdout);
	...
	while (getline(cin, input_string)) {
		if (input_string[0] == 'i') {
			...
			std::cout << "insert state = " << (take ? "true" : "false") << '\n';
			// step++;
			// printGraph(G, step);
			// print_graph(G);
			// printForest(P2);
